...
func TestAppendOpSequence(t *testing.T) {
	s := newTestStore(t)
...
func TestGetOpsSinceAndRange(t *testing.T) {
	s := newTestStore(t)
	for i := 0; i < 5; i++ {
...
func TestSnapshots(t *testing.T) {
	...
	if atSeq != 2 || string(state) != "state-2" {
		t.Fatalf("snapshot = %q @ %d, want state-2 @ 2", state, atSeq)
	}
}
...
// TestConcurrentAppendNoGaps verifies sequence assignment is gap-free and
